...
function formatICP(data) {
  const lines = [
    ...
    `Company Size:     ${data.clientCompanySize || ''}`,
    `Client Revenue:   ${data.clientRevenue || ''}`,
    `Buyer Titles:`,
    `  ${data.buyerTitles || ''}`,
    `Geography:        ${data.clientGeography || ''}`,
    `Client Model:     ${data.clientModel || ''}`,
    '',
